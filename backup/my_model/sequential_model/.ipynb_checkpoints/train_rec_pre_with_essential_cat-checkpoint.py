        ...
        for stepi,step in enumerate(u):#对用户每个时间步修改
            for ci,c in enumerate(step):#对每个预测位置修改，已经reindex
                if ci in cat_reindex_id:#当预测索引在所有目录重索引中时
                    ...
                    cat_second_id=Find_second_by_cat_id(c_id,second_keys,dic_id_second)#将对应id(hex)映射到二级目录(hex)
...
for epoch in range(epoch_size):
    ...
    for batch_step,(batch_users,batch_cat_id,batch_delta_t,
        ...
        outputs=rec_pre_model(batch_users,batch_cat_id,batch_delta_t,batch_delta_d)#(batch_size,padded_seqlen,tgt_size)
        #计算一个批次的损失
        ...
        batch_outputs_list_with_importance=Checkin_cat2importance(outputs,dic_cat_id,dic_id_second,dic_second_id,
                                                                  second_importance,batch_step,batch_size)
        #3对每个用户计算真实的时间步
