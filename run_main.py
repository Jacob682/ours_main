...
from utils.utils import accuracy, MRR, to_cuda, exe_time, save_checkpoint, load_checkpoint, get_current_branch, save_model_res, uncertain_weight
...
def run_pref_austgn(batch_size, num_epoch, delta, num_layers, num_x, lr, weight_decay, \
    ...
    for epoch in range(epoch_buck + 1, num_epoch):
        ...
        for batch, batch_inputs in enumerate(tra_inputs):
            ...
            outputs, shuffle_indices, b_seq_loss = model(model_inputs, num_neg[0]) #(batch_size, neg_num, 1)
            ...
            b_avg_poi_loss, b_avg_cat_loss = b_seq_loss
            b_avg_loss = uncertain_weight(b_avg_loss) + uncertain_weight(b_avg_poi_loss) + uncertain_weight(b_avg_cat_loss)
            # 反向传播
            b_avg_loss.backward()
            optimizer.step()
        ...
        if epoch % 1 == 0:
            with torch.no_grad():
                ...
                for batch_step, batch_inputs in enumerate(tst_inputs):
                    ...
                    neg_inputs = batch_inputs[27:]
                    model_inputs = [austgn_inputs, pref_inputs, y_inputs, neg_inputs]
                    outputs, shuffle_indices, b_seq_loss = model(model_inputs, num_neg[1])
