...
def fun_save_data(dir_data, batch_size, dir_output):
    if os.path.exists(dir_output):
        return
    else:
        tra_dataloader = Process_prefstgn_data(dir_data, batch_size)
        with open(dir_output, 'wb') as f:
            pickle.dump(tra_dataloader, f)
    return



def run_prefstgn(batch_size,patience,delta,num_layers,num_x,dropout,lr,weight_decay,
                 pref_embs,stgn_embs,mlp_units,dir_inputs,dir_outputs,len_tra,len_tes,num_negs,head_num):
    '''
    dropout:无用
    '''
    ...
    fun_save_data(dir_inputs, batch_size, dir_outputs)
    ...
    for epoch in range(patience):
        ...
        tra_inputs,tes_inputs = pickle.load(open(dir_outputs, 'rb'))
...
def main_nyc():
    ...
    dir_outputs = '/data/liuqiuyu/POI/old_model/data_process_nyc_dataloader_40.pkl'
