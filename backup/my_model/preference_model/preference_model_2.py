...
class Preference_Model(nn.Module):
    ...
    def __init__(self,embed_size_poi,embed_size_cat_id,
                 ...
                 num_layers,mlp_units,dropout=0.1):
        '''
...
pre_preference_train_state_data=Process_preference_model_train_state(train_x_poi_int,train_x_cat_int,train_x_day,train_x_hour,train_x_hs5_int,train_x_days_subs,
                                        negs_poi_int,negs_cat_int,
                                        train_y_poi_int,train_y_cat_int,train_y_day,train_y_hour,train_y_hs5,batch_size)
...
for epoch in range(epoch_size):
    model.train()
    epoch_loss=0.0
    for batch_step,(tra_x_poi_int,tra_x_cat_int,tra_x_day,tra_x_hour,tra_x_hs5_int,tra_x_days_subs,
                    neg_poi_int,neg_cat_int,tra_y_poi_int,tra_y_cat_int,tra_y_day,tra_y_hour,tra_y_hs5) in enumerate(pre_preference_train_state_data):
        len_batch=len(tra_x_poi_int)
        model.zero_grad()
        loss=0.0
        
        tra_x_poi_int=tra_x_poi_int.cuda()
        tra_x_cat_int=tra_x_cat_int.cuda()
        tra_x_day=tra_x_day.cuda()
        tra_x_hour=tra_x_hour.cuda()
        tra_x_hs5_int=tra_x_hs5_int.cuda()
        tra_x_days_subs=tra_x_days_subs.cuda()
        neg_poi_int=neg_poi_int.cuda()
        neg_cat_int=neg_cat_int.cuda()
        tra_y_poi_int=tra_y_poi_int.cuda()
        tra_y_cat_int=tra_y_cat_int.cuda()
        tra_y_day=tra_y_day.cuda()
        tra_y_hour=tra_y_hour.cuda()
        tra_y_hs5=tra_y_hs5.cuda()

        outputs=model(tra_x_poi_int,tra_x_cat_int,tra_x_day,tra_x_hour,tra_x_hs5_int,tra_x_days_subs,
                        neg_poi_int,neg_cat_int,tra_y_poi_int,tra_y_cat_int,tra_y_day,tra_y_hour,tra_y_hs5)#(bs,sq,21)
        #计算loss
        outputs=outputs.unsqueeze(-1)#(bs,sq,21,1)
        labels=torch.ones_like(outputs)#(bs,sq,21,1)
